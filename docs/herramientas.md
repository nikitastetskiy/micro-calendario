...
No podemos considerar a los *test* como una *herramienta* ya que se interpreta a los test como parte integral del proceso de desarrollo y el código. Aún así, `Jest` es un framework de testing generalista que podemos utilizar en cualquier situación, en el cual podemos crear, ejecutar y estructurar pruebas. El lado positivo de estas pruebas es que puedes tener control sobre el funcionamiento de lo que estas creando, puedes incluso realizar diferentes tipos de pruebas a un mismo bloque de código y de esta manera puedes saber que tan susceptible es esa parte de código.

La principal ventaja es que supone un buen flujo de trabajo con `Npm` y `Node`. Además posee una gran documentación y comunidad, lo cual hace mas facil y agil su aprendizaje. Tampoco hace falta una biblioteca de aserciones, ya que esta incluida.